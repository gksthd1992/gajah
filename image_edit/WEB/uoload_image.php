...
if ( isset($_FILES["file"]["type"]) )//변수가 있는지 없는지
{
  $max_size = 5000 * 10240; // 500 KB
  $destination_directory = "upload/";
  $validextensions = array("jpeg", "jpg", "png");

  $temporary = explode(".", $_FILES["file"]["name"]);//.을 기준으로 배열 저장
  $file_extension = end($temporary);//제일끝에 배열을 file_extension에 저장 
       ...
       ) && in_array($file_extension, $validextensions))
  {
    if ( $_FILES["file"]["size"] < ($max_size) )
    {
      ...
      {
        echo "<div clasbtn btn-lg btn-primarys=\"alert alert-danger\" role=\"alert\">Error: <strong>" . $_FILES["file"]["error"] . "</strong></div>";
      }
      else
      {
